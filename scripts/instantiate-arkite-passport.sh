...
for ENV in "osmosis" "stargaze"; do
    ...
    MSG="'{\"default_token_uri\": \"$DEFAULT_TOKEN_URI\", \"escrowed_token_uri\": \"$ESCROWED_TOKEN_URI\", \"transferred_token_uri\": \"$TRANSFERRED_TOKEN_URI\", \"cw721_base\": {\"code_id\": $CODE_ID_CW721, \"admin\": {\"instantiator\": {}}, \"label\": \"Arkite InterChain Passport collection\", \"msg\": \"$CW721_MSG\"}, \"cw721_poap\": {\"code_id\": $CODE_ID_CW721, \"admin\": {\"instantiator\": {}}, \"label\": \"Arkite InterChain Passport collection\", \"msg\": \"$POAP_MSG\"},\"ics721_base\": {\"code_id\": $CODE_ID_ICS721, \"admin\": {\"instantiator\": {}}, \"label\": \"Arkite InterChain Passport collection\", \"msg\": \"$ICS721_MSG\"}}'"
    CMD="$CLI tx wasm instantiate $CODE_ID_ARKITE_PASSPORT "$MSG" --from $WALLET_ARKITE_PASSPORT --label 'Ark test' --admin $WALLET_ARKITE_PASSPORT --gas-prices $CLI_GAS_PRICES --gas $CLI_GAS --gas-adjustment $CLI_GAS_ADJUSTMENT -b $CLI_BROADCAST_MODE --yes --node $CHAIN_NODE --chain-id $CHAIN_ID --output $CLI_OUTPUT"
